...
// Robust binding for "Voir plus" Academic button
(function () {
    function initAcademicShowMore() {
        const btn = document.getElementById('academic-show-more');
        const extras = document.querySelectorAll('[data-academic-extra-card]');
        if (!btn || !extras || extras.length === 0) return;

        let expanded = false;
        btn.setAttribute('aria-expanded', 'false');
        btn.setAttribute('aria-controls', 'academic-grid');
        btn.addEventListener('click', (e) => {
            expanded = !expanded;
            extras.forEach(card => {
                if (expanded) card.classList.remove('hidden');
            ...
            const textSpan = btn.querySelector('[data-i18n="academic.showMore"]') || btn.querySelector('span');
            const iconSpan = btn.querySelector('span:last-child');
            if (expanded) {
                if (textSpan) { textSpan.setAttribute('data-i18n', 'academic.showLess'); textSpan.textContent = 'Voir moins'; }
                if (iconSpan) iconSpan.textContent = '▲';
                btn.setAttribute('aria-expanded', 'true');
            } else {
                if (textSpan) { textSpan.setAttribute('data-i18n', 'academic.showMore'); textSpan.textContent = 'Voir plus'; }
                if (iconSpan) iconSpan.textContent = '▼';
                btn.setAttribute('aria-expanded', 'false');
            }
        });
    ...
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', initAcademicShowMore);
    else initAcademicShowMore();
})();
